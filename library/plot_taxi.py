...
class Plot:
    ...
    def __init__(self, buf, output, distance_name):
        ...
        """
        df1 = pd.read_table(buf, error_bad_lines= False, skiprows= 1, names= list(range(13)))

        df1 = df1.iloc[:, 0:14]
        df1.columns= [str(x) for x in range(1, 14)]
        xx= list(df1.loc[(df1['12'] == "K2P distance") & (df1['13'] == "p-distance with gaps")].index)
    ...
    def calculations(self):
        with open(os.path.join(self.output, "statistics_distance.txt"), mode='a') as file:
            df= self.table
            ...
            for i, col in enumerate(df2.columns):
                ...
                sd= sd.rename(index={"count": 'Count', "std": 'StdDev', "mean":  'Mean', "min": 'Min', '25%':'25% Quartile', '50%':'Median', '75%':'75% Quartile', "max":"Max"})
            ...
            for i, col in enumerate(df2.columns):
                for name, group in df[['comparison_type', col]].groupby('comparison_type'):
                    group= group.set_index('comparison_type')
                    df222 = pd.DataFrame(group.describe().squeeze())
                    df222= df222.rename(index={"count": 'Count', "std": 'StdDev', "mean":  'Mean', "min": 'Min', '25%':'25% Quartile', '50%':'Median', '75%':'75% Quartile', "max":"Max"})

                    df222[col]= df222[col].map(lambda x: round(x, 3)).map(lambda x: str(x))
                    new= df222.loc["Count", col]
                    cc= int(float(new))
                    df222.at["Count", col] = cc
                    df222.loc["Count", col] = cc
                    df222.replace(new, cc)
                    print(f"Descriptive Statistics for {col} and and comparison_type {name}", file = file)
            ...
            for i, col in enumerate(df2.columns):
                for name, group in self.genus[["genus (query 1)", 'comparison_type', col]].groupby(["genus (query 1)", 'comparison_type']):
                    ...
                    df222.loc["Count", col] = cc
                    df222.replace(new, cc)
                    print(f"Descriptive Statistics for {col} and Genus {name[0]} and comparison_type {name[-1]}", file = file)
                    print(df222, file= file)
            ...
            for i, j in zip(Man1.columns, Man2.columns):
                print(f"Mann-Whitney-U test Statistics for interspecies and intraspecies distance values", file = file)

                stat, p = mannwhitneyu(Man1[i], Man2[j])

                print('Statistics U (Z-value)=%.3f, p=%.3f' % (stat, p), file= file)
                print('################################################################################################', file= file)
                print(f'difference between maximum intraspecies and minimum inerspecies is {Man2[j].max()- Man1[i].min()}', file= file)
                print(f'Total number of intraspecific distance values that are higher than the lowest interspecific value and their percentage relative to the total number of intraspecific values is {Man2[Man2[j]> Man1[i].min()].index.size}, and {(Man2[Man2[j]> Man1[i].min()].index.size)/(Man2.index.size)} respectively', file= file)
                print(f'Total number of interspecific distance values that are lower than the highest intraspecific value, and their percentage relative to the total number of interspecific values is {Man1[Man1[i]> Man2[j].max()].index.size}, and {(Man1[Man1[i]> Man2[j].max()].index.size)/(Man1.index.size)} respectively', file= file)




if __name__ == "__main__":
    processplot('requirements_alignments.txt', os.path.join(".", "output"), ['p-distance', "K2P distance"])
